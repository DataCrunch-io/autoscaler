...
import (
	...
	dcStrings "k8s.io/autoscaler/cluster-autoscaler/cloudprovider/datacrunch/datacrunch-sdk-go/internal/strings"
	"k8s.io/autoscaler/cluster-autoscaler/cloudprovider/datacrunch/datacrunch-sdk-go/private/protocol"
)
...
func unmarshalHeaderMap(r reflect.Value, headers http.Header, prefix string, normalize bool) error {
	...
	case map[string]*string: // we only support string map value types
		...
		for k, v := range headers {
			if dcStrings.HasPrefixFold(k, prefix) {
				if normalize == true {
					k = strings.ToLower(k)
				} else {
